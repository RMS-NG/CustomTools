...
const ChartContainer = forwardRef(
  (
    ...
    const exportPDF = async (canvas, exportFilename) => {
      ...
      doc.addImage(await canvas.toDataURL("image/jpeg", 1.0), "JPEG", 0, 0);
      doc.save(exportFilename + ".pdf");
    };

    const exportPNG = async (canvas, exportFilename) => {
      ...
      //   window.navigator.msSaveBlob(await canvas.msToBlob(), exportFilename + ".png");
      // } else {
      //   setDataURL(await canvas.toDataURL());
    ...
    useImperativeHandle(ref, () => ({
      exportTo: async (exportFilename, exportFileextension) => {
        ...
        await html2canvas(chart.current, {
          ...
          async canvas => {
            if (exportFileextension.toLowerCase() === "pdf") {
              await exportPDF(canvas, exportFilename);
            } else {
              await exportPNG(canvas, exportFilename);
            }
            setExporting(false);
